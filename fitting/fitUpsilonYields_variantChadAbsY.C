...
//this "variant of fitUpsilonYields" replaces dimuYMin and dimuYMax by dimuPtMin and dimuPtMax, forcing the fit to be done in -2.4<y<2.4
//the reason for this is the number of arguments one can append to RooDataSet (max. is 8)
//
void fitUpsilonYields_variantChadAbsY(int choseSample    = 3, //Input data sample.  1: pp@7TeV data ; 2: pp@2.76TeV; 3: PbPb@276
			      ...
			      )
{
  ...
  TString cut_ap(Form("(%d<=Centrality && Centrality<%d) && ((%.2f<= (upsRapidity) && (upsRapidity)<=%.2f)||(upsRapidity<= %.2f && upsRapidity>=%.2f)) && (%.2f<muMinusEta && muMinusEta < %.2f) && (%.2f<upsPt && upsPt<%.2f) && vProb > %.2f",centrality_min,centrality_max,upsYCut_min,upsYCut_max,upsYCut_NegMax,upsYCut_NegMin,muonEtaCut_min,muonEtaCut_max,dimuPtMin,dimuPtMax,vProbPick));
  ...
  cout<<"Fitting: Pt["<< dimuPtMin <<","<< dimuPtMax <<"] and centrality ["<<centrality_min<<","<<centrality_max<<"]!!!!"<<endl;
  ...
  RooDataSet *data0 = new RooDataSet("data0","data0",theTree,
				     RooArgSet(*mass,*upsPt,*muPlusPt,*muMinusPt,*upsRapidity,*muMinusEta,*Centrality,*vProb));
  ...
  RooDataSet *likesignData0    = new RooDataSet("likesignData0","likesignData0",allsignTree,
						RooArgSet(*mass,*upsPt,*muPlusPt,*muMinusPt,*upsRapidity,*muMinusEta,*Centrality,*QQsign,*vProb));
  ...
  if (doConstrainFit) 
    {
      ...
      fit_2nd      = pdf->fitTo(*data,Constrained(),Save(kTRUE),Extended(kTRUE),Minos(doMinos),NumCPU(4));
    }
  else 
    {
RooAbsPdf  *pdf             = new RooAddPdf ("pdf","total p.d.f.",
 ...
 fit_2nd       = pdf->fitTo(*data,Save(kTRUE),Extended(kTRUE),Minos(doMinos));
  ...
  latex1.DrawLatex(0.15,1.-0.05*3.5,Form("%.1f< |y| < %.1f",upsYCut_min,upsYCut_max)); 
