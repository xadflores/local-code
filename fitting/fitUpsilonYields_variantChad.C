...
//
void fitUpsilonYields_variantChad(int choseSample    = 3, //Input data sample.  1: pp@7TeV data ; 2: pp@2.76TeV; 3: PbPb@276
			      ...
			      )
{
  ...
  double upsYCut_min    = -1.5;
  double upsYCut_max    = -1.0;
  
  int centrality_max = centralityMax; 
  int centrality_min = centralityMin; 
  
  TString cut_ap(Form("(%d<=Centrality && Centrality<%d) && (%.2f<upsRapidity && upsRapidity < %.2f) && (%.2f<muMinusEta && muMinusEta < %.2f) && (%.2f<upsPt && upsPt<%.2f) && vProb > %.2f",centrality_min,centrality_max,upsYCut_min,upsYCut_max,muonEtaCut_min,muonEtaCut_max,dimuPtMin,dimuPtMax,vProbPick));
