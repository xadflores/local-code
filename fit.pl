...
my $pdfPrefix = "pdfOutput/2507/";
my $txtPrefix = "txtOutput/2507/";
...
my $samstart = 3;
my $samend   = 3;
my @choseSamples = ("","pp7tev-d0", "pp7tev-d3","pbpbRegit_nf","pbpbPPrecoTT", "pbpbPPrecoGG","pbpbZhen2011","pp2p76tev");
# ------------------------------------------------------
my $doNarrowMass = 1; 
my $choseFitParams = 0; #what you want 0: (1s, 2s, 3s) 1: (1s, 2s/1s; 3s/1s); 2: (1S, (2s+3s)/1s); 3:(1S, 2S/1S, 3S/1S, 3S/2S)
my @choseWhat2Fit = ("yield","ratio1","ratio23","ratio32");
...
my @centrality_max = ("40","2","4","8","12","16","20","40","8","36");
...
my @rapBinMin = ("-2.4","-1","1");
my @rapBinMax = ("-1","1","2.4");

# pT binning:
my @upsPtBinMin = ("0","3","6","10");
my @upsPtBinMax = ("3","6","12","150");
my $dontDoRapNow =1;
my $dontDoPtNow =0;
	...
	for (my $ivProb=$vProbStart; $ivProb<=$vProbEnd; $ivProb++)
	{
		...
		for (my $isig=$sigstart; $isig<=$sigend; $isig++)
		{
			...
			"); 
			    ...
			    for(my $iUpsPt=0; $iUpsPt<=3; $iUpsPt++){ #hardcoded... so what?
			...
			"); 
			    ...
			    for ( my $ibkg=$modelstart; $ibkg<=$modelend; $ibkg++)
			    {
				next if($dontDoRapNow==0 && $dontDoPtNow==0);
				my $centMin = $centrality_min[$icent];
				my $centMax = $centrality_max[$icent];
				my $myfsr   = $fsr[$ifsr];
				my $myref   = $useRef[$iref];
